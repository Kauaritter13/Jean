...
async function scrapeHavan(url: string): Promise<ProductData | null> {
  try {
    ...
    // Extract product title
    const titleMatch = html.match(/<h1[^>]*class="[^"]*product-name[^"]*"[^>]*>([^<]+)<\/h1>/) ||
                       html.match(/<h1[^>]*>([^<]+)<\/h1>/)
    const title = titleMatch ? titleMatch[1].trim() : null
    ...
    // Extract image
    const imageMatch = html.match(/<img[^>]*class="[^"]*product-image[^"]*"[^>]*src="([^"]+)"/) ||
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url || !listId) {
      return NextResponse.json({ error: 'URL e listId sao obrigatorios' }, { status: 400 })
    }
